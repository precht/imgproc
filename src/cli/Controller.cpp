...
using namespace imgproc::core;
using namespace imgproc::opencv;
...
Controller::Controller(int argc, char** argv)
    : handlers_(loadHandlers())
    , description_(loadDescritpion())
    , helper_(std::make_shared<OpenCVImageHelper>())
    , img_(helper_)
{
    po::store(po::parse_command_line(argc, argv, description_), used_options_);
...
void Controller::run()
{
    // if help option display message and finish
    if (used_options_.count(HELP)) {
        cout << DESCRIPTION_INTRO << description_ << endl;
        return;
    }

    // if gui is displayed then wait() should be called twice: before and after performing operations on images
    if (used_options_.count(GUI)) is_gui = true;

    // load main image
    if (inputs_.size() == 0 || !used_options_.count(INPUTS))
        throw std::logic_error("no input file");
    img_.load(inputs_.front());
    if (is_gui) windows_.createWindow(MAIN_WINDOW);
    if (is_gui) windows_.show(img_, MAIN_WINDOW);

    // handle options
    for(auto it : used_options_) {
        auto h = handlers_.find(it.first);
        if (h != handlers_.end()) (this->*(h->second))();
    }

    // save main image
    if(used_options_.count(OUTPUTS) && outputs_.size() >= 1) {
        img_.save(outputs_[0]);
    }
    else {
        img_.save("a_0.bmp");
    }
    if (is_gui) {
        windows_.show(img_, MAIN_WINDOW);
        windows_.wait();
        windows_.closeAll();
    }
}

po::options_description& Controller::loadDescritpion()
{
    auto& desc = *new po::options_description("DESCRIPTION");
    desc.add_options()
            (HELP ",h", "produce help message")
            (GUI ",g", "display gui")
            (INPUTS ",i", po::value<vector<string>>(&inputs_), "input files")
            (OUTPUTS ",o", po::value<vector<string>>(&outputs_), "output files")
            (FFT, "fast Fourier transform")
            (FREQ_DOMAIN, po::value<vector<string>>(), FREQ_DOMAIN_MSG)
    ;
    return desc;
}

std::map<string, void(Controller::*)(void)>& Controller::loadHandlers()
{
    // create map with functions pointer associated with options names
    auto& m = *new std::map<string, void(Controller::*)(void)>();
    m.insert(make_pair<string, void(Controller::*)(void)>(FFT, &Controller::fftHandler));

    return m;
}

void Controller::fftHandler()
{
    auto mats = FrequencyUtils::fastFourierTransform(img_);
    freqDomainHandler(*(mats.get()));
    if (is_gui) windows_.wait();
    FrequencyUtils::inverseFastFourierTransform(img_, *(mats.get()));
}

void Controller::freqDomainHandler(const vector<boost::numeric::ublas::matrix<std::complex<double>>>& mats)
{
    // if no freq-domain option then do nothing
    if (!used_options_.count(FREQ_DOMAIN))
        return;

    const vector<string>& vals = used_options_[FREQ_DOMAIN].as<vector<string>>();

    const int rows = (int)mats.front().size1();
    const int cols = (int)mats.front().size2();
    const int channels = (int)mats.size();
    const string name1 = vals.size() < 2 ? "a_1.bmp" : vals[1];
    const string name2 = vals.size() < 3 ? "a_2.bmp" : vals[2];

    Image output1(rows, cols, channels, helper_, "", name1);
    Image output2(rows, cols, channels, helper_, "", name2);

    core::ConvertType type = CT_PHASE_MAGNITUDE; // default type
    if (vals.size() >= 1 && vals.front().compare("ri") == 0) type = CT_REAL_IMAGINARY;

    FrequencyUtils::complexMatrixToImages(mats, output1, output2, type);

    output1.save();
    output2.save();

    // if is_gui then display show windows with proper descriptions
    if (is_gui && type == CT_PHASE_MAGNITUDE) {
        if (!windows_.existWindow(PHASE_WINDOW)) windows_.createWindow(PHASE_WINDOW);
        if (!windows_.existWindow(MAGNITUDE_WINDOW)) windows_.createWindow(MAGNITUDE_WINDOW);
        windows_.show(output1, PHASE_WINDOW);
        windows_.show(output2, MAGNITUDE_WINDOW);
    }
    if (is_gui && type == CT_REAL_IMAGINARY) {
        if (!windows_.existWindow(REAL_WINDOW)) windows_.createWindow(REAL_WINDOW);
        if (!windows_.existWindow(IMAGINARY_WINDOW)) windows_.createWindow(IMAGINARY_WINDOW);
        windows_.show(output1, REAL_WINDOW);
        windows_.show(output2, IMAGINARY_WINDOW);
    }
}
...
//            }
//            else if (it->first.compare(FFT_S) == 0)
//            {
