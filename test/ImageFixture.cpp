...
bool ImageFixture::checkEqual(const unsigned char* data, Image& img, int x, int y, int ch, bool use_test_check)
{
    ...
    for(int i = 0; i < x; ++i)
        for(int j = 0; j < y; ++j)
            for(int k = 0; k < ch; ++k)
            {
                is_correct &= data[(i * y * ch) + (j * ch) + k] == img(i, j, k);
                if (use_test_check) BOOST_CHECK_EQUAL(data[(i * y * ch) + (j * ch) + k], img(i, j, k));
            }
